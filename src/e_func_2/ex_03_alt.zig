...
const Tuple = struct {bool, u32, u32};
...
fn divide(m: u32, n: u32, d: u32) Tuple {
    const divide_m = @mod(m, d) == 0;
    const divide_n = @mod(n, d) == 0;
    const novo_m = if (divide_m) @divTrunc(m, d) else m;
    const novo_n = if (divide_n) @divTrunc(n, d) else n;
    return .{divide_m or divide_n, novo_m, novo_n};
}
